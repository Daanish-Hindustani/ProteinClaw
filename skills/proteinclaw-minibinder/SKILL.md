...
description: Detailed ProteinClaw de-novo minibinder workflow and MCP tool guidance.
---
...
1. **ProteinClaw domain MCP tools** (`proteinclaw_run_create`,
   ...
   `proteinclaw_<category>_<tool>` such as
   `proteinclaw_design_rfdiffusion3`). **Canonical for every pipeline
   ...
   shell scripts. Use `proteinclaw_data_pdb_analyze` for structured
...
2. **Native agent tools** supplied by Codex, Claude Code, or another MCP-capable agent (`Bash`, `Read`,
...
#### 1a. Inspect the PDB
`proteinclaw_data_pdb_fetch` with ONLY `pdb_id=...`
...
2. **Structural sandbox analysis.** First call
   `proteinclaw_data_pdb_analyze` on the cropped / co-crystal PDB
...
## Self-evolution (optional — promote durable lessons to skills)

Your `plan.md` is run-local; it dies with the run. When you learn something
**durable and generalizable** — a tool footgun and its fix, a better default,
a strategy that worked for a fold/target class, or a correction to stale
guidance — you MAY promote it into the **global ProteinClaw skill files** so
every future run benefits. This is **optional and rare**: do it only when the
lesson would change how a future run behaves, never for run-specific facts
(those stay in `plan.md`). If nothing durable was learned, change nothing.

**When:** at a round boundary, right after you update `plan.md`, or after final
report generation. At most **one** skill evolution action per round.

**What you may change (and ONLY these):**
- **Tool-specific guidance** → patch or rewrite the relevant
  `proteinclaw-tool-*` skill.
- **General technique / target-class playbook** → create or rewrite a focused
  `proteinclaw-learned-<short-topic>` skill.
- **Stale or misleading learned guidance** → patch it, replace it with a clean
  corrected skill, or delete it if it should no longer influence future runs.

**How — clean procedural memory, not append-only logs:**
- Prefer `proteinclaw_skill_patch` for a precise correction to one block.
- Prefer `proteinclaw_skill_write` when the clean final skill should be a
  rewritten `SKILL.md` rather than a pile of appended notes.
- Use `proteinclaw_skill_create` for a new focused learned skill.
- Use `proteinclaw_skill_delete` only when a ProteinClaw learned/tool skill is
  obsolete or actively harmful. Do not delete core workflow skills unless the
  user explicitly asks.
- `SKILL.md` content must start with YAML frontmatter containing matching
  `name:` and non-empty `description:`.
- Keep each skill concise and procedural. Do not store raw run logs, target
  facts, long traces, or one-off conclusions.
- If provenance helps, include a short line such as
  `Learned from run <run_id>, <YYYY-MM-DD>:` inside the relevant section, but
  do not preserve every historical edit as an append-only changelog.
- **Perform the edit, THEN report it — never the reverse.** Actually call the
  scoped ProteinClaw skill MCP tool and confirm success. Only after a successful
  tool call may you mention the exact ProteinClaw skill changed. The run summary
  and report derive from real tool calls in the trace; claimed-but-unmade skill
  edits are correctness failures.
- Edits take effect on the **next** run, not the current one.

The human reviews the git diff and commits or reverts intended skill changes.
Write each skill as if a maintainer will read the diff — because they will.
...
|---|---|---|---|
| `data.rcsb_search` | `proteinclaw_data_rcsb_search` | 1 | — |
| `data.pdb_fetch` | `proteinclaw_data_pdb_fetch` | 1 | — |
| `data.pdb_analyze` | `proteinclaw_data_pdb_analyze` | 1/8 | — |
