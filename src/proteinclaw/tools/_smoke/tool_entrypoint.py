#!/usr/bin/env python3
"""Universal container entrypoint shim (PRD §9.2).

Identical across every GPU tool — do not customise this file per tool.

Reads kwargs from ``$INPUT_FILE`` (default ``/workspace/input.json``), calls
``implementation.run(**kwargs)``, writes the result envelope to
``$OUTPUT_FILE`` (default ``/workspace/output.json``).

Errors are caught and serialised as the standard error envelope rather than
re-raised; an uncaught exception would mean the host runner sees a nonzero
exit with no parsable output, which is harder to debug.
"""
