...
def test_wizard_supports_gemini_env_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    ...
    assert result.config.ai_provider == "gemini"
    assert result.config.ai_api_key == "gemini-from-env"
    assert result.config.ai_model == "gemini-2.5-flash"


def test_wizard_supports_openrouter_env_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    ...
    prompter = _Prompter(
        [
            "openrouter",
            "y",  # use env var? yes
            "",  # accept default OpenRouter model
            "n",  # install? no
        ]
    )
    secret = _Prompter([])
    ...
    assert result.config.ai_provider == "openrouter"
    assert result.config.ai_api_key == "openrouter-from-env"
    assert result.config.ai_model == "google/gemini-2.5-flash"
